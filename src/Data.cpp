...
//        replicas[device] = arr;
    ...
    DataBasePtr DataImpl::Renew(size_t bytes) const {
        DataBasePtr ret = std::make_shared<DataImpl>(mm_, bytes);
        ret->PinnedToDevice(device_pinned_, device_pinned_strict_);
        LG(DEBUG) << "DataImpl Renew " << *this;
        return ret;
    }
}
